...
function render() {
    ...
    if (mousebutton === MOUSE.DOWN && mouse_index !== -1) {
        const tileIndex = (_a = inventory.get(mouse_index)) === null || _a === void 0 ? void 0 : _a.tileIndex();
        if (tileIndex) {
            drawTile({
                position: new Vector({
...
function drawTile({ position, size, tileIndex }) {
    ctx.drawImage(tileset, Math.floor(tileIndex % 64) * size.x, Math.floor(tileIndex / 64) * size.y, size.x, size.y, position.x, position.y, size.x, size.y);
}
